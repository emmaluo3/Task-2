...
class ChatLevelFeaturesCalculator:
    ...
    def apply_politeness(self) -> None:
        ...
        """
        # politeness_features_df = pd.DataFrame()
        # for index, row in self.chat_data.iterrows():
            #text= row['message']
            #politeness_df = get_politeness_strategies(text)

        #pd.concat([self.chat_data, politeness_features_df], axis=1)
        ...
        # new_df = get_politeness_strategies(self.chat_data["message"].iloc[0])
